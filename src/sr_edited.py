...
class LoopbackOutputAudio(AudioSource):
    """
    Creates a new ``Microphone`` instance, which represents a physical microphone on the computer. Subclass of ``AudioSource``.
    ...
    Higher ``chunk_size`` values help avoid triggering on rapidly changing ambfient noise, but also makes detection less sensitive. This value, generally, should be left at its default.
    """
    def __init__(self, device_index=None, sample_rate=None, chunk_size=1024):
        assert device_index is None or isinstance(device_index, int), "Device index must be None or an integer"
        assert sample_rate is None or (isinstance(sample_rate, int) and sample_rate > 0), "Sample rate must be None or a positive integer"
        ...
        self.pyaudio_module = self.get_pyaudio()
        audio = self.pyaudio_module.PyAudio()
        p = self.pyaudio_module.PyAudio()
        try:
            wasapi_info = p.get_host_api_info_by_type(self.pyaudio_module.paWASAPI)
        except OSError:
        ...
        self.format = self.pyaudio_module.paInt16  # 16-bit int sampling
        self.SAMPLE_WIDTH = self.pyaudio_module.get_sample_size(self.format)  # size of each sample
        self.SAMPLE_RATE = int(default_speakers["defaultSampleRate"])  # sampling rate in Hertz
        self.CHUNK = int(default_speakers["defaultSampleRate"] * chunk_duration)  # number of frames stored in each buffer
    ...
    def __enter__(self):
        ...
        self.audio = self.pyaudio_module.PyAudio()
        try:
            self.stream = LoopbackOutputAudio.MicrophoneStream(
                self.audio.open(
                    input_device_index=self.device_index, channels=2, format=self.format,
                    rate=self.SAMPLE_RATE, frames_per_buffer=self.CHUNK, input=True,
                )
            )
        except Exception:
